...
def _date_filter(key, df, default_days=365):
    ...
    with c1:
        preset = st.selectbox("기간", list(presets.keys()), index=3, key=f'{key}_preset')
    with c2:
        days = presets[preset]
...
def _nearest_date(df: pd.DataFrame, cat: str, dt) -> pd.Timestamp | None:
    avail = df[df['category'] == cat]['date'].unique()
    if len(avail) == 0:
        return None
    dt = pd.Timestamp(dt)
    return min(avail, key=lambda x: abs((pd.Timestamp(x) - dt).days))
...
def _render_summary_table(df: pd.DataFrame):
    ...
    with cf1:
        sel_cats = st.multiselect("표시 계열", all_cats,
            ...
            key='table_cats')
    with cf2:
        ref_cat = st.selectbox("스프레드 기준", all_cats,
    ...
    for cat in sel_cats:
        ...
        for tn in target_tenors:
            ...
            sp = (cur - ref_y[tn]) * 100 if not np.isnan(cur) and not np.isnan(ref_y.get(tn, np.nan)) else None
    ...
    for r in rows:
        ...
        for tn in target_tenors:
            ...
            if v is None:
                ...
                html += f'<td class="neg">({abs(v):.1f})</td>'
            else:
                html += f'<td class="pos">{v:.1f}</td>'
        html += "</tr>"
    html += "</tbody></table>"
...
def _render_spread_chart(df: pd.DataFrame):
    ...
    if show_fill:
        fig.add_trace(go.Scatter(x=merged['date'], y=merged['spread_bp'],
            name='스프레드(bp, 우축)', fill='tozeroy',
            fillcolor='rgba(221,232,192,0.35)',
    ...
    if show_fill and show_avg:
        ...
        fig.add_trace(go.Scatter(
            x=[merged['date'].min(), merged['date'].max()], y=[avg_sp, avg_sp],
...
# ── 탭3: 커브 & 월간변동 ──────────────────────────────────────────
def _single_curve_mom(df: pd.DataFrame, cat: str, d1_str: str, d2_str: str):
    d1, d2 = pd.Timestamp(d1_str), pd.Timestamp(d2_str)

    def get_cv(dt):
        s = df[(df['category'] == cat) & (df['date'] == dt)]
        if len(s) == 0:
            nd = _nearest_date(df, cat, dt)
            if nd is None:
                return pd.DataFrame()
            s = df[(df['category'] == cat) & (df['date'] == nd)]
        s = s.copy()
        s['t_ord'] = s['tenor'].map(TENOR_ORDER_MAP)
        return s.sort_values('t_ord')

    cv1, cv2 = get_cv(d1), get_cv(d2)
    common_t = [t for t in TENOR_LABELS
                if t in cv1['tenor'].values and t in cv2['tenor'].values]
    if not common_t:
        st.warning("공통 만기 데이터 없음")
        return
    ...
    # 텍스트 위치: 절댓값이 max의 35% 이상이면 inside(흰색), 아니면 outside(검정)
    max_abs = max(abs(v) for v in mom_bp) if mom_bp else 1
    inside_idx  = [i for i, v in enumerate(mom_bp) if abs(v) >= max_abs * 0.35]
    outside_idx = [i for i, v in enumerate(mom_bp) if abs(v) < max_abs * 0.35]

    fig = make_subplots(specs=[[{'secondary_y': True}]])

    def _bar(indices, tpos, tcolor):
        if not indices:
            return
        fig.add_trace(go.Bar(
            x=[common_t[i] for i in indices],
            y=[mom_bp[i] for i in indices],
            marker_color=[bar_colors[i] for i in indices],
            opacity=0.82,
            text=[f"{mom_bp[i]:+.1f}" for i in indices],
            textposition=tpos,
    ...
    _bar(inside_idx, 'inside', '#ffffff')
    _bar(outside_idx, 'outside', '#333333')

    # 범례용 더미
    fig.add_trace(go.Bar(x=[None], y=[None], name='전월대비 변동(bp)',
                         marker_color='#4A5E35', showlegend=True), secondary_y=False)

    if len(cv1) > 0:
        cv1f = cv1[cv1['tenor'].isin(TENOR_LABELS)]
        fig.add_trace(go.Scatter(
            x=cv1f['tenor'], y=cv1f['yield'], name=f'{d1_str}(우축)',
            mode='lines+markers', line=dict(color='#4A5E35', width=2),
            marker=dict(size=7), hovertemplate='%{x}: %{y:.3f}%<extra></extra>'),
            secondary_y=True)

    if len(cv2) > 0:
        cv2f = cv2[cv2['tenor'].isin(TENOR_LABELS)]
        fig.add_trace(go.Scatter(
            x=cv2f['tenor'], y=cv2f['yield'], name=f'{d2_str}(우축)',
...
def _render_curve_mom(df: pd.DataFrame):
    ...
    for idx, col in enumerate(cols):
        with col:
            ...
            d2_str = st.selectbox("비교일", date_strs,
                                  index=min(21, len(date_strs) - 1), key=f'cv_d2_{idx}')
...
def render(df: pd.DataFrame):
    ...
    with tab2:
        _render_spread_chart(df)
    with tab3:
        _render_curve_mom(df)
    with tab4:
